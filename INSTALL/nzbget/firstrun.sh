...
if [ -f /config/nzbget.conf ]; then
  ...
  chown swuser:swuser /config/nzbget.conf
  mkdir -p /downloads/dst
  chown -R swuser:swuser /downloads
fi

# Verify and create come directories
if [[ ! -e /config/queue ]]; then
  mkdir -p /config/queue
  chown -R swuser:swuser /config/queue
fi

if [[ ! -e /config/log ]]; then
  mkdir -p /config/log
  chown -R swuser:swuser /config/log
fi

if [[ ! -e /config/ppscripts ]]; then
  mkdir -p /config/ppscripts
  chown -R swuser:swuser /config/ppscripts
fi
